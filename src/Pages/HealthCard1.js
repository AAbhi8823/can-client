  ...
  const handleOnSubmit = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        formData2.append("emergencyContacts", JSON.stringify(emergencyContacts));

        await axios.post(
          `${baseurl}/api/add_emergencyContacts`,
          formData2,
          {
            headers: {
              Authorization: `Bearer ${token}`,
        ...
        <div className="h-fit w-full bg-[#FEF8FD] flex flex-col gap-4 pb-5 justify-center items-center lg:px-20 px-3">
          ...
          <div className="h-full bg-white rounded-xl py-2">
            <form className="flex flex-wrap items-center justify-center gap-7 mx-[4%] my-4">
              ...
              {["gender", "blood_group", "cancer_type", "cancer_stage", "current_treatment"].map((key) => (
                <div className="md:w-[46%] w-full relative group" key={key}>
                  ...
                  >
                    ...
                    {key === "blood_group" && ["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"].map(option => (
                      <option value={option} key={option}>{option}</option>
                    ))}
                    {key === "cancer_type" && ["Breast Cancer", "Lung Cancer", "Prostate Cancer","Colon Cancer","Lymphoma","Melanoma","Pancreatic Cancer","Liver Cancer","Other"].map(option => (
                      <option value={option} key={option}>{option}</option>
                    ))}
                    {key === "cancer_stage" && ["Stage 0", "Stage IA", "Stage IB", "Stage IIB","Stage IV"].map(option => (
                      <option value={option} key={option}>{option}</option>
                    ))}
                    {key === "current_treatment" && ["Chemotherapy", "Radiotherapy", "Surgery", "Immunotherapy"].map(option => (
                      <option value={option} key={option}>{option}</option>
                    ))}
                  </select>
                  <label
              ...
              {showEmergencyContacts && emergencyContacts.map((contact, index) => (
                ["name", "phone"].map(field => (
                  <div className="md:w-[46%] w-full relative group" key={`${field}_${index}`}>
                    <input
                      type="text"
                    ...
                    {errors[`emergency_${field}_${index}`] && (
                      <div className="text-red-500 text-xs mt-1">{errors[`emergency_${field}_${index}`]}</div>
                    )}
                  </div>
                ))
              ))}
              ...
              {[PDF, PDF2, PDF3].map((pdf, index) => (
                <div className="w-full md:w-[46%] h-24 relative group" key={index}>
                  <div
                    ...
                    onClick={() => uploadPDF(index === 0 ? hiddenChoosePDF : index === 1 ? hiddenChoosePDF1 : hiddenChoosePDF2)}
                  >
                    {pdf ? (
                      <>
                      ...
                      <>
                        ...
                        <div>Upload {index === 0 ? "Aadhaar Card" : index === 1 ? "Fit to Fly Certificate" : "Biopsy Certificate"}</div>
                      </>
                    )}
                  </div>
                  <input
                    type="file"
                    ref={index === 0 ? hiddenChoosePDF : index === 1 ? hiddenChoosePDF1 : hiddenChoosePDF2}
                    onChange={(e) => handlePDFUpload(e, index === 0 ? setPDF : index === 1 ? setPDF2 : setPDF3)}
                    style={{ display: "none" }}
              ...
              <button
                type="button"
                className={`md:w-[25%] w-full h-12 rounded-xl ${isSubmitting ? "bg-[#F5F5F5] cursor-not-allowed" : "bg-[#B30F73] text-white"}`}
