...
const AddProfile = () => {
  ...
  const mainPin = async () => {
    setIsVerifying(true);
    try {
      const dataPin = await axios.post(`${baseurl}/user/user-register`, {
        pin: loginPin,
      });
  ...
  const handleRepinChange = (value) => {
    setRepin(value);
    setError(pin === value ? "Pins are equal" : "Pins do not match, Please re-enter the pin correctly!");
  };
  ...
  return (
    <>
      {creatPin_open && (
        <div className="fixed inset-0 flex items-center justify-center lg:p-0 p-2 bg-black bg-opacity-50 z-50 " style={{ backdropFilter: "blur(2px)" }}>
          <div className="bg-white h-max  rounded-3xl">
            <div className="relative flex lg:py-6  py-4 lg:px-4 items-center flex-col">
              ...
              <p className=" text-[14px]  text-center px-3 pt-2">Its your space, so add a profile lock to keep your account information with you</p>
              <div className=" text-2xl py-1 flex flex-row items-center justify-center w-full gap-4 ">
                ...
                <PinInput length={4} id="pin" value={pin} onComplete={handlePinChange} inputStyle={{ border: "none", borderBottom: "1px solid black", width: "30px", textAlign: "center", marginRight: "30px" }}></PinInput>
              </div>
              <div className=" text-2xl py-1 flex flex-row items-center justify-center gap-4 w-full  ">
                ...
                <PinInput length={4} id="repin" value={repin} onComplete={handleRepinChange} inputStyle={{ border: "none", borderBottom: "1px solid black", width: "30px", textAlign: "center", marginRight: "30px" }}></PinInput>
              </div>
              <div className="w-[40%] pt-5 flex flex-col items-center  ">
                {error && <p className="text-red-500 text-center">{error}</p>}
                <div className="flex flex-col justify-end pt-7">
                  <button className={`w-48 h-12 bg-[#efc31a33] font-bold rounded-[20px] text-white items-center mx-auto ${error && error === "Pins are equal" ? "bg-[#efc31a]" : "pointer-events-none"}`} disabled={error && error !== "Pins are equal"}>Continue</button>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 flex items-center justify-center lg:p-0 p-2 bg-black bg-opacity-50 z-50  " style={{ backdropFilter: "blur(2px)" }}>
          <div className="bg-white h-max lg:w-[30%] w-[340px] rounded-3xl">
            <div className="relative flex py-6  items-center flex-col">
              ...
              {errMsg && <div className="text-red-400 lg:text-xs">Please Enter the correct pin!</div>}
              <div className="text-2xl py-6 flex flex-row items-center justify-center w-full">
                <p className="text-xs  font-semibold flex justify-center w-[14%] pt-8 ">PIN</p>
                <PinInput length={4} id="pin" value={loginPin} onComplete={handlePinChange1} inputStyle={{ border: "none", borderBottom: "1px solid black", width: "30px", textAlign: "center", marginRight: "30px" }}></PinInput>
              </div>
              <div className="flex justify-end w-[80%] pt-4">
                {loginPin.length === 4 ? (
                  <div className="bg-[#C31A7F] text-center px-6 py-2 cursor-pointer rounded-xl text-white" onClick={mainPin}>{isVerifying ? "Verifying..." : "Continue"}</div>
                ) : (
        ...
        <div className="flex lg:flex-row lg:p-0 p-2 items-center justify-center">
          ...
          <div className="md:w-1/2 lg:w-[35%] px-5">
            <form className="bg-white shadow-md rounded  rounded-2xl  mb-4">
              <div>
                <img src={vtwo} className="w-full w-[100%] rounded-[20px]" alt="none" />
              </div>
              ...
              <div className="h-[60%] w-[100%] mt-2 relative">
                <div className="flex justify-center  pt-[65px] pb-[65px] ">
                  {perData && perData.map((userget, i) => (
                    <div key={i} onClick={() => { setUserId(userget._id); getUser(userget._id); }} className="w-[141px] mb-1 h-[130px] rounded-3xl mx-2 flex flex-col justify-center items-center cursor-pointer" style={{ boxShadow: "0px 10px 30px rgba(0, 0, 0, 0.05)", background: userBoxId === userget._id ? "#c31a75" : "#FEE5EA", color: userBoxId === userget._id ? "white" : "black" }}>
                      ...
                      <div className="absolute top-[28px] rounded-full overflow-hidden bg-white p-[3px]">
                        <Avatar alt=" " src={userget.profile_image} sx={{ width: 56, height: 56 }} />
                      </div>
                    </div>
                  ))}
